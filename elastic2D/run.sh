...
nz=323         # vertical samples in models
dx=6.250       # horizontal discretization parameter 
dz=6.658       # vertical discretization parameter
...
pgcc -acc -fast -ta=tesla,managed elasticIsotropic2D_offshore.c -lm -o run.exe
./run.exe $parFileName $xrecPath $xsrcPath $sourcePath $vp $vs $rho $dampPath 
rm run.exe

# transp n1=$spread n2=$nt <data/seism.bin | ximage n1=$nt d1=$dt n2=$spread  perc=99 &
